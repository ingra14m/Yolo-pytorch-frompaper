...
class yoloLoss(nn.Module):
    def __init__(self, S, B, l_coord, l_noobj, device, grid_num=7.):
    ...
    # 这个iou经过验证是正确的！！需要提供左上角和右下角的坐标即可
    ...
    def forward(self, pred_tensor, target_tensor):
        ...
        coo_mask = target_tensor[:, :, :, 4] > 0  # 是否为物体, [16, 7, 7]的维度
        noo_mask = target_tensor[:, :, :, 4] == 0  # 是否为背景
        coo_mask = coo_mask.unsqueeze(-1).expand_as(target_tensor)  # [16, 7, 7, 30] 30维度根据mask全部为True or False
        noo_mask = noo_mask.unsqueeze(-1).expand_as(target_tensor)

        coo_pred = pred_tensor[coo_mask].view(-1, 30)  # 把所有batch的所有grid展开
        box_pred = coo_pred[:, :10].contiguous().view(-1, 5)  # box[x1,y1,w1,h1,c1]，把每一个grid的两个判断框展开
        ...
        for i in range(0, box_target.size()[0], 2):  # choose the best iou box
            ...
            # box1_xyxy[:, :2] = box1[:, :2] / self.grid - 0.5 * box1[:, 2:4]
            box1_xyxy[:, :2] = box1[:, :2] - 0.5 * box1[:, 2:4]
            box1_xyxy[:, 2:4] = box1[:, :2] + 0.5 * box1[:, 2:4]
            box2 = box_target[i].view(-1, 5)  # ground truth
        ...
        # 我觉得这里不应该是box_target_response_iou, 这个得到的是iou向量
        ...
        # 这个源代码原本是2 * contain_loss，我认为并不合理
